...
Extracts benchmark runtimes and SPECrate scores from extracted results or tar archives.
Supports GCC, Clang, and ICC across 32-bit and 64-bit builds with -O0, -O1, -O2, -O3 optimizations.
Outputs structured CSV and JSON files in results/.
"""
...
def save_results(records, output_dir='results'):
    ...
    # Save CSV
    csv_file = os.path.join(output_dir, 'spec_cpu2017_results.csv')
    fieldnames = ['benchmark', 'compiler', 'bitness', 'optimization', 'runtime_seconds', 'spec_rate', 'run_id', 'suite']
    ...
    # Save JSON
    json_file = os.path.join(output_dir, 'spec_cpu2017_results.json')
